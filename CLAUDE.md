...
- **Interface CG production config**: preconditioner `'auto'` resolves to `two_level[deflated](jacobi+PoU)` for CG+tilewise, rtol 1e-8 (≤1 µV accuracy budget; 1e-12 = bit-identical validation grade). Never run a **block_jacobi base** at the split regime: never-assemble BJ blocks are single-owner-tile `S_i` slices (not true diagonal blocks) and cold CG stagnates — and the BJ byte-budget guard is a *memory* guard, not a numerics guard, so it may not downgrade for you (the BRCM silent-hang incident). Force the base explicitly with `--interface-two-level-base jacobi` (CLI/YAML since 2026-08-03) and bound `--interface-cg-maxiter` (default 3·n turns stagnation into a multi-day silent hang). Details: `src/distributed/CLAUDE.md` + `docs/brcm_distributed_runtime_optimization.md` §7.13–§7.15.
