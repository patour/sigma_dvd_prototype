...
from model.unified_model import UnifiedPowerGridModel, LayerID
from model.edge_adapter import EdgeInfoExtractor
...
class UnifiedIRDropSolver:
    ...
    Example usage:
        ...
        # To use cholmod backend globally (if sksparse installed):
        from solver.unified_solver import set_use_cholmod
        set_use_cholmod(True)  # or False to force splu, or None for auto
    """

    def __init__(self, model: UnifiedPowerGridModel, verbose: bool = False):
        ...
        Args:
            ...
            verbose: If True, print timing info for matrix factorization.
                     Also sets verbose on the model if not already set.
        """
        self.model = model
        self._verbose = verbose
        # Propagate verbose to model if requested
        if verbose and not model._verbose:
            model._verbose = True
    ...
    def _build_coupled_preconditioner(
        ...
        if preconditioner_type == 'amg':
            from solver.coupled_system import AMGPreconditioner, HAS_PYAMG
            if not HAS_PYAMG:
                raise ImportError(
