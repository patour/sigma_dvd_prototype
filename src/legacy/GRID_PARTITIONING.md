...
from legacy import GridPartitioner
...
from legacy import PowerGridModel, IRDropSolver, StimulusGenerator
...
Comprehensive test suite available in `tests/legacy/test_partitioner.py`:

```bash
python3 -m unittest tests.legacy.test_partitioner -v
```
...
- `legacy/generate_power_grid.py`: Grid generation
- `notebooks/`: Interactive notebook examples
