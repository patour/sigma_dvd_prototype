...
## Overview
Created a new module `legacy/effective_resistance.py` (originally `irdrop/effective_resistance.py`) that computes effective resistance between node pairs in the power grid network. The implementation is optimized for batch computation to efficiently handle large numbers of node pairs.

## Module: `legacy/effective_resistance.py`
...
```python
from legacy import EffectiveResistanceCalculator
...
### Usage
See the Usage Example section above and `tests/legacy/test_irdrop.py` for
comprehensive usage patterns including batch computation and symmetry validation.

## Package Integration

Exported from `legacy/__init__.py`:
```python
...
from legacy import PowerGridModel, EffectiveResistanceCalculator
...
### Key Files:
- `legacy/effective_resistance.py` - Main module implementation
- `EFFECTIVE_RESISTANCE_SUMMARY.md` - This document

### Tests:
- `tests/legacy/test_irdrop.py` - TestEffectiveResistance test class (12 tests)
