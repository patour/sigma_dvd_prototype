...
- **Preconditioner**: `'auto'` → `two_level` for CG+tilewise, else `block_jacobi`. Production split-regime config: `two_level[deflated](jacobi+PoU)` at rtol 1e-8 (measured 166 nV vs the ≤1 µV accuracy budget; rtol 1e-12 is bit-identical validation grade). The two_level base is selected by `interface_two_level_base` — CLI `--interface-two-level-base {auto,block_jacobi,jacobi,neumann}` / `solver:` YAML (since 2026-08-03); **pass `jacobi` explicitly on production split-regime runs** — `'auto'` is the legacy bj-with-byte-budget-downgrade path (the §7.13 hazard below). GenEO default `geneo_k=0` (measured zero benefit); `interface_warm_start_extrapolation` opt-in (`2·x_prev − x_prev2`).
...
- The BJ byte-budget guard (`interface_block_jacobi_max_bytes`) is a **memory guard, not a numerics guard**: on BRCM the 3.1 GiB estimate sits below the 4 GiB auto floor, the jacobi downgrade cannot fire, and the run silently built the stagnating bj base. Fix: `--interface-two-level-base jacobi --interface-cg-maxiter 2000` (the historical `--interface-block-jacobi-max-bytes 1` downgrade-forcing workaround still works but is obsolete).
- Default `maxiter = 3·n_interface` turns "did not converge" into a **multi-day silent hang** — bound it on production-scale runs and set `--interface-cg-progress-every 50` (`interface_cg_progress_every` in YAML/settings, since 2026-08-03; also still assignable post-construction as `cg_solver.progress_every`) for true-residual logging: healthy solves (<50 iters) never log, a stagnating one shows its plateau live.
